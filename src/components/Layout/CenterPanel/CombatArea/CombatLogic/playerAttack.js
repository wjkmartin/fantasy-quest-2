...
export function onClickAttackSquare(
  dispatch,
  player,
  items,
  targetObj,
  ...
  dispatch(
    UI.actions.setActorAttackAnimation({ actorId: 0, direction: direction() })
  );
  dispatch(
    actor.actions.setActorAttributeByActorId({
      actorId: 0,
  ...
  const equippedItemsPlayer = items.itemsById.filter(
    (item) => item.equipped === true
  );
  const playerEquippedWeapon =
    equippedItemsPlayer.find((item) => item.slot === 'weapon_main') ||
    'unarmed';
  ...
  const abilityTotalDamage =
    ability.damage - targetObj.armor >= 0
      ? ability.damage - targetObj.armor
      : 0;
  const enemyHealthAfterAttack = targetObj.health - abilityTotalDamage;
  dispatch(
    actor.actions.setActorAttributeByActorId({
      actorId: targetObj.id,
      attribute: 'health',
      value: enemyHealthAfterAttack < 0 ? 0 : enemyHealthAfterAttack,
    })
  );
  dispatch(
    UI.actions.addMessageToActivityLog({
      message: `Your attack deals ${abilityTotalDamage} damage!`,
    })
  );
  ...
  if (enemyHealthAfterAttack <= 0 && !isDuel) {
    // kill enemy logic
    UI.actions.addMessageToActivityLog({
      message: `${targetObj.actorName} dies horribly!`,
    });
    // TODO: verify drops
    targetObj.drops?.forEach((drop) => {
      if (_.random(1, 100) <= drop.chance) {
        ...
        dispatch(
          item.actions.createNewItem({
            actorId: 0,
            item: new Item(drop.item_type, drop.item, -1, currentLocation.name),
          })
        );
      }
    });
    ...
    UI.actions.addMessageToActivityLog({message: `You defeated ${targetObj.actorName}!`, styleType: 'green'});
    dispatch(combat.actions.removeActorFromCombatById(targetObj.id));
    dispatch(UI.actions.setActiveItemOrNpcTarget({ id: null, type: null }));
  }
